...
echo "=== [004/006] Creating secret 'onepassword-connect-token' ==="
kubectl create secret generic --namespace onepassword onepassword-connect-token \
  --from-literal=token="$jwt"

echo "=== [005/006] Creating secret 'connect-server-credentials' ==="
kubectl create secret generic --namespace onepassword connect-server-credentials \
